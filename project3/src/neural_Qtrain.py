...
def init(env, env_name):
    ...
    2. return:
       state_dim
       action_dim:
    3. cartpole, mountain car: left unchanged
    4. hints:
       1) 您将需要修改此函数来离散操作空间，并创建从动作索引到操作的
         全局字典映射（可以在`get_env_action（）'中使用）
       2) 慎重考虑action space
       3) 创建 a global dictionary mapping from action index to action:
          能用在 `get_env_action()`
          
       3) 对于Pendulum-v0`env.action_space.low [0]`
        和`env.action_space.high [0]`是动作空间的限制
       4) 设置 global flag, 能用在 `get_env_action()`
          设置一个全局标志是连续的，您可以在`get_env_action（）'
          中使用可能有助于将相同的代码用于离散和（离散）连续动作空间
    """
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    2. 参数:
       state_dim
       action_dim
       hidden_nodes = HIDDEN_NODES = 5
    3. 建议结构: 每个output node 代表一个 Q value for one action
       e.g.: 2 output node for carpole, 前进， 后退？
    4. hints:
       1) 给定如何在RL中使用q值，是否有必要具有输出激活功能？
       2) 您将在`get_train_batch`中进一步放置`target_in`。在执行损失之前可能最好实施（有进一步的提示）
    """
...
"""
不用写： 通过q值，取得max的值，获得下一步的动作
get_action = egreedy_action 
"""
...
"""
3.要写：修改您已经离散的连续动作空间，请参阅`init（）`中的提示
"""
...
"""
4.要写=文章的perceive
  perceive中一个最重要的事情是，存储，然后根据情况进行train。
  我们要求只存储数据>batch的大小，就开始train
"""
...
def qtrain(env, state_dim, action_dim,
    ...
    for episode in range(num_episodes):
        ...
        for step in range(ep_max_steps):
            ...
            # 这里是 egreedy_action , 这几行，就是 文章中的DQN
